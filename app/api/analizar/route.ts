...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!materialId) return NextResponse.json({ ok: false, error: 'Falta materialId' }, { status: 400 });
    if (!tipoProducto) return NextResponse.json({ ok: false, error: 'Falta tipoProducto/contexto' }, { status: 400 });

    const bridgeUrl = process.env.ARKON_BRIDGE_URL;
    const bridgeToken = process.env.ARKON_BRIDGE_TOKEN;
    if (!bridgeUrl) return NextResponse.json({ ok: false, error: 'ARKON_BRIDGE_URL no está configurada en Vercel' }, { status: 503 });
    if (!bridgeToken) return NextResponse.json({ ok: false, error: 'ARKON_BRIDGE_TOKEN no está configurado en Vercel' }, { status: 503 });
    ...
    if (!response.ok || !data.ok) {
      return NextResponse.json({ ok: false, error: data.error || 'El motor ARKON devolvió un error' }, { status: response.status || 502 });
    }

    return NextResponse.json({ ok: true, resultado: data.resultado, fuente: 'ARKON_DAP_REAL' });
  } catch (error: any) {
